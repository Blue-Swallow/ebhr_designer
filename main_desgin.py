...
def read_cond():
    ...
    fldname = input("Please input the name of folder which contains calculating condition such as {}\n>>".format(name_cond_py))
    if os.path.exists(fldname):
        if os.path.exists(os.path.join(fldname, name_cond_py)):
            ...
            import cond
            dic_cond = {"PARAM_EX": cond.PARAM_EX,
                        "PARAM_LIQUID": cond.PARAM_LIQUID,
                        "PARAM_CEA": cond.PARAM_CEA,
                        "CONST_VF": cond.CONST_VF
                        }
        else:
